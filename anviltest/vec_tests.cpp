...
TEST(EqTest, FloatEquality) {
    ...
    EXPECT_TRUE(floateq(std::numeric_limits<double>::max(), std::numeric_limits<double>::max()));
}
...
TEST(Vec4fTest, Mag) {
    ...
    f64_t expected = static_cast<f64_t>(std::sqrt(14));
    EXPECT_DOUBLE_EQ(mag(v1), expected);
    EXPECT_DOUBLE_EQ(mag(v1), mag(v2));
}
...
TEST(Vec4fTest, Norm) {
    ...
    Vec4f n = norm(v);
    f64_t test = mag(n);
    EXPECT_DOUBLE_EQ(test, 1.0);
    EXPECT_DOUBLE_EQ(n.x, 1.0f / static_cast<f64_t>(std::sqrt(14)));
    EXPECT_DOUBLE_EQ(n.y, 2.0f / static_cast<f64_t>(std::sqrt(14)));
    EXPECT_DOUBLE_EQ(n.z, 3.0f / static_cast<f64_t>(std::sqrt(14)));
...
TEST(Vec4fTest, Dot) {
    ...
    EXPECT_DOUBLE_EQ(dot(Vec4f(1.0, 2.0, 3.0, 4.0), Vec4f(1.0, 2.0, 3.0, 1.0)), 18.0);
}
...
TEST(Vec4fTest, Cross) {
    ...
    EXPECT_DOUBLE_EQ(expected.x, -1.0);
    EXPECT_DOUBLE_EQ(expected.y, 2.0);
    EXPECT_DOUBLE_EQ(expected.z, -1.0);
